...
np.random.seed(0)
NUMBER_OF_OBS = 2
INPUT_LAYER = np.random.randn(1, NUMBER_OF_OBS * 4)
weights = np.random.randn(NUMBER_OF_OBS * 4, 1)
print(weights)
print(INPUT_LAYER)
print(sigmoid(np.dot(INPUT_LAYER, weights)))
...
class PLAYER():
    ...
    def give_answer(self,data):
        sig=sigmoid(np.dot(INPUT_LAYER,weights))
        if sig>0.5:
            emote.jump()
        else:
            emote.fall()